...
def parse_response(response_bl, response_st, DCT_VAR_ID_TO_LABEL):
    ...
    df_bl = parse_flat_data(df_bl, DCT_VAR_ID_TO_LABEL)

    # The response is a JSON and it must be flattened into a table
    df_st = flatten_bulk_raw_response(response_st.json()['results'], LST_HEADERS)

    # Some parsing is required, including pivoting by variable
    df_st = parse_flat_data(df_st, DCT_VAR_ID_TO_LABEL)
...
def post_process_data(df, DCT_INTERVALS_TO_DISCARD):
    ...
    df = subset_drop_first_n_rows(df, subset_a='device', subset_b='variable', n_rows=1)
...
def double_subset_rolling_clean(df, subset_a=None, subset_b=None, clean_on=None):
    ...
    for item_a in set(df[subset_a]):
        ...
        for item_b in set(df_subset_a[subset_b]):
            ...
            df_clean.sort_values(by=[df_clean.index.name, subset_a, subset_b], inplace=True)
            df_clean.drop(columns='primary_key', inplace=True)

            is_outlier = (df_clean['outlier']==True)

            if (SMOOTHING_METHOD == 'median'):
                df_clean.loc[~is_outlier, clean_on] = df_clean.loc[~is_outlier, clean_on].rolling(window=SMOOTHING_WINDOW, center=True).median()
            elif (SMOOTHING_METHOD == 'mean'):
                df_clean.loc[~is_outlier, clean_on] = df_clean.loc[~is_outlier, clean_on].rolling(window=SMOOTHING_WINDOW, center=True).mean()
...
def get_coords_dataframe(LST_DEVICE_ID_TO_REQUEST):
    ...
    df_coords = Ubidots.get_gps_for_multiple_device_id(LST_DEVICE_ID_TO_REQUEST, _TOKEN)
