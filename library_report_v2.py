...
class Processing:
    ...
    # TODO: add method to add simultaneous time series (like power)
...
class Cleaning:
    ...
    def remove_outliers_by_zscore(df, zscore=None):
        lst_df = []
        for device in set(df['device']):
            df_dev = df.query("device == @device")
            for variable in set(df_dev['variable']):
                df_subset = df_dev.query("variable == @variable").copy()

                z_scores = stats.zscore(df_subset['value'], nan_policy='omit')
                lst_df.append(df_subset[abs(z_scores) < zscore])

        return pd.concat(lst_df)


    def remove_lower_quantile(df, quantile=None):
        lst_df = []
        for device in set(df['device']):
            df_dev = df.query("device == @device")
            for variable in set(df_dev['variable']):
                ...
                q_low = df_subset['value'].quantile(q=quantile)
                lst_df.append(df_subset[df_subset['value'] > q_low])
...
class Graphing:
    ...
    def plot_typical_day_by_hour(df_subset, subset=None, title=None, bl_label=None, bl_ci_label=None, legend=False, include_ci=False, fill_ci=True):
        idx = 0
        fig = go.Figure()
        for subset_period in set(df_subset[subset]):
            ...
            if (include_ci is True):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(dash='dash'),
                    showlegend=legend,
                    name=f"{bl_ci_label} {subset_period}"
                ))
    ...
    def plot_typical_week_by_day(df_subset, subset=None, title=None, bl_label=None, bl_ci_label=None, legend=False, include_ci=False, fill_ci=True):
        idx = 0
